...
const AestheticMode = ({post, user, seeMore, handleSeeMore, openComment}) => {
  return (
    <View style={styles.container}>
      ...
      <PostButton post={post} openComment={openComment} />
...
const PostImages = ({post}) => {
  ...
  let onScroll = e => {
    let pageNumber = Math.min(
      Math.max(
        Math.floor(e.nativeEvent.contentOffset.x / (width - 24 * 2) + 0.5),
        0,
      ),
...
const PostButton = ({post, openComment}) => {
  return (
